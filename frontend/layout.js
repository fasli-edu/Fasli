...
(function () {
  ...
  window.renderShell = function (opts) {
    ...
    function resyncLayoutOnViewportChange() {
      ...
      // ✅ (طلب — مؤكَّد بسكرين شوت حقيقي) رسوم Chart.js (توزيع الطلاب/الحضور الأسبوعي...)
      // كانت بتطلع أكبر من حاويتها ومقطوعة برة حدود الشاشة بعد دورة تدوير كاملة — Chart.js
      // بيعتمد على ResizeObserver داخلي عشان يعيد حساب مقاس الرسمة تلقائيًا مع responsive:true،
      // لكن اللحظة اللي الـobserver بيتنفّذ فيها ممكن تسبق لحظة استقرار تخطيط الصفحة النهائي
      // بعد التدوير (سباق توقيت)، فالرسمة بتاخد مقاس غلط وتفضل عالقة عليه. الحل: نجبر كل
      // رسمة موجودة فعليًا في الصفحة تعيد حساب مقاسها يدويًا من غير ما ننتظر الـobserver
      if (window.Chart && window.Chart.instances) {
        Object.values(window.Chart.instances).forEach(function (chart) {
          try { chart.resize(); } catch (e) { /* تجاهل */ }
        });
      }
    }
