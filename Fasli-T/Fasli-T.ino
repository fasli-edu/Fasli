...
void handleNotFound() {
  ...
  // hotspot-detect.html من آيفون) وصل للجهاز أصلاً ولا لأ. لو اتصلتي ومفيش أي سطر
...
void startCaptivePortal() {
  ...
  // ✅ (تخفيف بعد ملاحظة عملية) وضع الإعداد بيشغّل نقطة وصول + محاولة اتصال STA خلفية في
  // نفس الوقت — ده استهلاك تيار لحظي أعلى بكتير من التشغيل العادي (STA بس)، ولاحظنا إعادة
  // تشغيل صامتة للبورد أثناء اختبار طويل (من غير أي رسالة كراش، يعني الأرجح انقطاع تيار
  // brownout مش خطأ في الكود) على بورد متغذّي من منفذ USB في كمبيوتر — ده مصدر تيار غير
  // مضمون كفاية لقفزات استهلاك ESP32+WiFi عمومًا. القوة القصوى (19.5dBm) كانت زيادة مش
  // ضرورية أصلاً لسيناريو الإعداد (الموبايل بيبقى جنب الجهاز مباشرة وقتها)، فرجّعناها
  // لمستوى أقل يقلل الحمل بدون ما يأثر على التغطية القريبة المطلوبة فعليًا
  WiFi.setTxPower(WIFI_POWER_15dBm);
  Serial.println("📶 تم ضبط قدرة إرسال الـ Wi-Fi على 15 dBm (تقليل حمل التيار في وضع الإعداد).");
